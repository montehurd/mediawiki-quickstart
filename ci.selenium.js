...
describe('Quickstart Selenium', () => {
    it('should show Version heading', async () => {
        await browser.url(`http://localhost:${process.env.MW_DOCKER_PORT}/wiki/Special:Version`);
        const heading = await $('#firstHeading');
        await expect(heading).toHaveText('Version');
    });

    it('should log in', async () => {
        await browser.url(`http://localhost:${process.env.MW_DOCKER_PORT}/w/index.php?title=Special:UserLogin`);
    ...
    it('appearance settings should have selected vector radio button', async () => {
        await browser.url(`http://localhost:${process.env.MW_DOCKER_PORT}/wiki/Special:Preferences#mw-prefsection-rendering`);
